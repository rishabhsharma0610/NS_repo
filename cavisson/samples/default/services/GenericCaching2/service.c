...
static int check_and_set_status_code(int *is_req_with_validator)
{
char query_val[4096], hdr_val[4096], http_hdr[4096];
int ret;

  *is_req_with_validator = 0;
  if((ns_http_req_has_validator_ims()) || (ns_http_req_has_validator_if_none_match()))
  {
    *is_req_with_validator = 1;
...
static void set_hdr_for_req_with_validator()
{
  ...
  check_and_set_http_hdr("CacheControl2", "Cache-Control: ");

  // Get value of query parameter - Cookie
  // This is to test set cookie
  check_and_set_http_hdr("Cookie2", "Set-Cookie: ");
}


static void set_hdr_for_req_without_validator()
{
  // Get value of query parameter - etag
  // We get Comma separated list of Etags in the ETag query paramter
  // e.g. ETag="etag1","etag2"
...
{
int is_req_with_validator;

  int code_filled = check_and_set_status_code(&is_req_with_validator);

  if(is_req_with_validator == 0)
    set_hdr_for_req_without_validator();
  else
    set_hdr_for_req_with_validator();
