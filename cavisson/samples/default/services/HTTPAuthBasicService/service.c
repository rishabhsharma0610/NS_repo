...
static void set_response()
{
  ...
  if(!strcmp(cookie_val, ""))
  {
    ...
    {
      ns_set_http_hdr("WWW-Authenticate: Basic");
    } else{
      ns_set_http_hdr("Set-Cookie: AuthMsg=1");
      ns_set_http_hdr("WWW-Authenticate: Basic YWRtaW5pc3RyYXRvcjpRQUBzZXJ2ZXI=");
    } 
  ...
  else if(!strcmp(cookie_val, "1"))
  {
	...
	if(!strcmp(test_case, "BasicRealmFail"))   
    {
      ns_set_http_status_code(401);
      ns_set_http_hdr("WWW-Authenticate: Basic");
    ...
    {
      if(!strcmp(test_case, "BasicRedirect"))
      {
        ns_set_http_status_code(301);
        set_location();
      } else if(!strcmp(test_case, "BasicStatus")){
        check_and_set_status_code();
      }else if(!strcmp(test_case, "BasicCacheable"))
      {
...
int HTTPAuthBasicService_on_request_callback() 
{
